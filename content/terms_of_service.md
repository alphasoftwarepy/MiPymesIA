# Términos de Uso - SG MiPymes IA

## 1. Aceptación de los Términos

Al acceder y utilizar SG MiPymes IA, usted acepta estar sujeto a estos Términos de Uso. Si no está de acuerdo con alguna parte de estos términos, no debe utilizar nuestro servicio.

## 2. Descripción del Servicio

SG MiPymes IA es una plataforma de generación de estrategias de marketing y publicidad impulsada por inteligencia artificial, diseñada para pequeñas y medianas empresas.

## 3. Registro y Cuenta de Usuario

- Debe proporcionar información precisa y completa durante el registro
- Es responsable de mantener la confidencialidad de su contraseña
- Debe notificar inmediatamente cualquier uso no autorizado de su cuenta

## 4. Planes y Suscripciones

- **Prueba Gratuita**: 7 días con 5 solicitudes
- **Planes Pagos**: Varían en duración y cantidad de solicitudes mensuales
- Los pagos son no reembolsables salvo lo establecido por ley

## 5. Uso Aceptable

Usted se compromete a:
- Utilizar el servicio solo para fines legales y comerciales legítimos
- No compartir su cuenta con terceros
- No intentar acceder a áreas restringidas del sistema
- No utilizar el contenido generado para actividades ilegales o no éticas

## 6. Propiedad Intelectual

- El contenido generado por la IA es de su propiedad para uso comercial
- La plataforma y su tecnología son propiedad de SG MiPymes IA
- No puede reproducir, modificar o distribuir la plataforma sin autorización

## 7. Limitación de Responsabilidad

SG MiPymes IA proporciona el servicio "tal cual" sin garantías de ningún tipo. No somos responsables de:
- Resultados comerciales obtenidos al implementar las estrategias
- Pérdidas financieras derivadas del uso del servicio
- Interrupciones temporales del servicio

## 8. Modificaciones

Nos reservamos el derecho de modificar estos términos en cualquier momento. Los cambios serán notificados a través de la plataforma.

## 9. Terminación

Podemos suspender o terminar su acceso si viola estos términos.

## 10. Contacto

Para consultas sobre estos términos, contacte a: alphasoftpy@gmail.com
